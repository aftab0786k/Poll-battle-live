...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    ...
    try {
      const res = await axios.post('https://poll-battle-live-7ja4.vercel.app/api/users/login', { email, password })
  ...
  const register = async (username, email, password) => {
    ...
    try {
      const res = await axios.post('https://poll-battle-live-7ja4.vercel.app/api/users/register', { username, email, password })
  ...
  const getUserStats = async () => {
    try {
      ...
      const res = await axios.get('https://poll-battle-live-7ja4.vercel.app/api/users/stats', {
        headers: { Authorization: `Bearer ${token}` }
      })
